...
def rename(folder):
    ...
    for _, filename in enumerate(os.listdir(folder)):
        ...
        if (not filename.startswith("._")) and (not filename.startswith("fantasy_")):

            dst = f"fantasy_{filenum}{extension}"
...
result = rename("./fantasy_worlds")
result = [x.replace("./fantasy_worlds/", "") for x in result]
